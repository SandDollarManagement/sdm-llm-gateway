...
export default function AppsClient({ initialApps }) {
  ...
  async function handleCreate(formData) {
    ...
    if (!res.ok) { showBanner('error', data.error || 'Failed to create app'); return }
  ...
  async function handleRotateToken(id, name) {
    if (!confirm(`Rotate bearer token for "${name}"? The old token stops working immediately and you'll need to update the app's env var.`)) return
    const res = await fetch(`/api/admin/apps/${id}/rotate-token`, { method: 'POST' })
    const data = await res.json()
    if (!res.ok) { showBanner('error', data.error || 'Failed to rotate'); return }
  ...
  return (
    <div className="max-w-4xl space-y-4">
      ...
      {newTokenInfo && (
        <TokenReveal info={newTokenInfo} onDismiss={() => setNewTokenInfo(null)} />
      )}

      <div className="flex justify-between items-center">
        <p className="text-sm text-muted">
          Each app gets a bearer token. Its consumer (Ops Hub, Media Manager, etc.) sends it as <code>Authorization: Bearer &lt;token&gt;</code>.
          Tokens are stored as SHA-256 hashes — you see the plaintext only once at creation or rotation.
        </p>
        <button
            ...
            {apps.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-muted text-sm">
                  No apps registered yet. Click "Register App" to add your first consumer (e.g. Ops Hub).
                </td>
              </tr>
            )}
            {apps.map(a => (
              <tr key={a.id} className="border-b border-border last:border-0">
                ...
                <td className="px-4 py-3 text-muted">
                  {a.monthly_budget_usd != null ? `$${Number(a.monthly_budget_usd).toFixed(2)}` : 'unlimited'}
                </td>
...
function TokenReveal({ info, onDismiss }) {
  ...
  async function copy() {
    try { await navigator.clipboard.writeText(info.token); setCopied(true); setTimeout(() => setCopied(false), 2000) } catch {}
  }
  return (
    <div className="bg-brand/10 border border-brand/40 rounded-xl p-5 space-y-3">
      ...
      <div className="text-xs text-muted">
        This is the only time you'll see the plaintext. Save it now — paste it into the consumer app's env var as <code>SDM_GATEWAY_TOKEN</code>.
      </div>
      ...
      <button onClick={onDismiss} className="text-xs text-muted hover:text-text-primary">Dismiss</button>
    </div>
  )
}
...
function AddForm({ onCancel, onSubmit }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-surface-card border border-border rounded-xl p-5 space-y-4">
      <div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-xs uppercase tracking-wide text-muted mb-1.5">Default Alias</label>
          <input
            type="text"
            value={defaultAlias}
            ...
            className="w-full bg-surface-tertiary border border-border rounded-md px-3 py-2 text-sm"
          />
          <p className="text-xs text-muted mt-1">Used when the app doesn't specify a model.</p>
        </div>
        <div>
          <label className="block text-xs uppercase tracking-wide text-muted mb-1.5">Monthly Budget (USD)</label>
          <input
            type="number"
            step="0.01"
            min="0"
      ...
      <div className="flex gap-2 justify-end pt-2">
        ...
        <button type="submit" className="px-4 py-2 rounded-md bg-brand hover:bg-brand-light text-white text-sm font-medium">Register App</button>
      </div>
    </form>
  )
}
...
function EditForm({ app, onCancel, onSubmit }) {
  ...
  const [budget, setBudget] = useState(app.monthly_budget_usd != null ? String(app.monthly_budget_usd) : '')
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <input type="text" value={name} onChange={e => setName(e.target.value)} className="bg-surface-tertiary border border-border rounded-md px-3 py-2 text-sm" />
        <input type="text" value={defaultAlias} onChange={e => setDefaultAlias(e.target.value)} className="bg-surface-tertiary border border-border rounded-md px-3 py-2 text-sm" />
        <input type="number" step="0.01" min="0" value={budget} onChange={e => setBudget(e.target.value)} placeholder="monthly budget USD" className="bg-surface-tertiary border border-border rounded-md px-3 py-2 text-sm" />
