...
export async function GET() {
  ...
  const rows = await query(
    `SELECT id, name, default_alias, monthly_budget_usd, enabled, created_at, last_used_at
       FROM apps
...
export async function POST(request) {
  ...
  const monthlyBudgetUsd = body?.monthly_budget_usd != null && body.monthly_budget_usd !== ''
  ...
  const dup = await queryOne(
    `SELECT id FROM apps WHERE workspace_id = $1 AND name = $2`,
    [WORKSPACE_ID, name]
  )
  if (dup) {
  ...
  const inserted = await queryOne(
    `INSERT INTO apps (workspace_id, name, token_hash, default_alias, monthly_budget_usd, enabled)
     VALUES ($1, $2, $3, $4, $5, true)
     RETURNING id, name, default_alias, monthly_budget_usd, enabled, created_at, last_used_at`,
    [WORKSPACE_ID, name, tokenHash, defaultAlias, monthlyBudgetUsd]
  )
