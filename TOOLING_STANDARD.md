...
## Node web app — required

**The kit DETECTS each project's real shape; it never forces one.** (This is the fix for
the 2026-07-04 incident, where a pnpm-forcing kit broke npm projects' `npm ci` deploys.)
Specifically the installer detects and honors:

- **Package manager** — read from the DEPLOY's own install command (parse the Dockerfile:
  `npm ci` vs `pnpm install`), NOT the lockfile or `packageManager` field (either can be
  stale/poisoned). It updates the RIGHT lockfile and never emits a foreign one. Mixed or
  ambiguous signals **STOP and surface** — the kit never guesses a package manager.
- **Language** — a `tsconfig.json` is **NEVER created**. Type-checking (`tsc --noEmit`) is
  wired only where a tsconfig already exists, so "JavaScript only / no tsconfig" projects
  (e.g. the gateway, Ops-Hub) are respected automatically — no `checkJs`, no conversion.
- **Linter** — an existing linter (ESLint flat/legacy, Biome, or `next lint`) is left
  untouched; a linter is seeded only when the project has none.
- **Node version** — read from the Dockerfile base image (normalized, e.g. `22.12.0-slim`
  → `22`); CI is rendered to match so CI and deploy never diverge.

The kit is proven by `standard-tooling/self-test/run.sh` (fixtures per shape + a real
`npm ci` proof) — run it before trusting the kit on a new shape.
...
"knip":      "knip"
"verify":    "<pm> run typecheck && <pm> run lint && <pm> run test && <pm> run build"
```

(`typecheck` is included only when a tsconfig exists.) `<pm> run verify` is the single
local "is this green?" command (`<pm>` = the detected npm or pnpm); CI runs the same set
plus E2E. The installer writes these with the detected package manager automatically.
...
`lint-staged.config.mjs`, and a rendered `.github/workflows/tooling-standard.yml` (warn-mode
quality + e2e jobs matching the detected package manager + Node; a hard build+test job is
added only when the project has no other CI).
